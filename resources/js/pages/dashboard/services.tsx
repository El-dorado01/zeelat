...
import { PaginatedResponse, type Services, type Alumni, type BreadcrumbItem } from '@/types';
...
const Services = () => {
    ...
    const initialTab = window.location.pathname === '/page-builder/services' ? 'services' : 'alumni';
    const [activeTab, setActiveTab] = useState(initialTab);

    // Update URL when tab changes
    const handleTabChange = (value: string) => {
        const newUrl = value === 'alumni' ? '/page-builder/alumni' : '/page-builder/services';
        router.replace({
            url: newUrl,
    ...
    useEffect(() => {
        const handlePopState = () => {
            const tab = window.location.pathname === '/page-builder/services' ? 'services' : 'alumni';
            setActiveTab(tab);
        };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={activeTab === 'alumni' ? 'Alumni' : 'Services'} />
